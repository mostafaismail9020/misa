...
public class ReCaptchaAspect
{
	...
	public Object advise(final ProceedingJoinPoint joinPoint) throws Throwable
	{
		...
		if (captchaEnabledForCurrentStore)
		{
			...
			if (request != null)
			{
				request.setAttribute("captchaEnabledForCurrentStore", Boolean.valueOf(captchaEnabledForCurrentStore));
				request.setAttribute("recaptchaPublicKey", getSiteConfigService().getProperty(RECAPTCHA_SITE_KEY_PROPERTY));
				 String recaptchaResponse = request.getParameter(RECAPTCHA_RESPONSE_PARAM);
				if(StringUtils.isEmpty(recaptchaResponse)) {
					recaptchaResponse = request.getHeader(RECAPTCHA_RESPONSE_PARAM);
				}
				...
				if (StringUtils.isBlank(recaptchaResponse) || !checkAnswer(recaptchaResponse))
				{
					...
					final RedirectAttributes redirectModel = (RedirectAttributes) CollectionUtils.find(args,
							PredicateUtils.instanceofPredicate(RedirectAttributes.class));

					if (redirectModel != null)
					{
						redirectModel.addAttribute("recaptchaChallangeAnswered", Boolean.FALSE);
					}
					...
					request.getSession().setAttribute("recaptchaChallangeAnswered", Boolean.FALSE);
	...
	protected boolean checkAnswer(final String recaptchaResponse)
	{
		...
		final HttpClientBuilder httpClientBuilder = HttpClients.custom().setRetryHandler(
