...
import com.investsaudi.constants.InvestsaudicustomerticketingConstants;
import com.investsaudi.forms.SupportTicketForm;
...
public class AccountSupportTicketsPageController extends AbstractSearchPageController
{
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.SUPPORT_TICKETS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.SUPPORT_TICKETS_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_HISTORY));
		...
		if (ticketAdded)
		{
			GlobalMessages.addConfMessage(model, InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_ADDED);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.ADD_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.ADD_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_ADD));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(InvestsaudicustomerticketingConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(InvestsaudicustomerticketingConstants.MAX_UPLOAD_SIZE_MB,
				FileUtils.byteCountToDisplaySize(maxUploadSizeValue));
		try
		{
			model.addAttribute(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_ASSOCIATED_OBJECTS,
					ticketFacade.getAssociatedToObjects());
			model.addAttribute(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_CATEGORIES, ticketFacade.getTicketCategories());
		}
			...
			final BindingResult bindingResult)
	{
		...
		if (bindingResult.hasErrors())
		{
			...
			list.add(buildMessageMap(InvestsaudicustomerticketingConstants.FORM_GLOBAL_ERROR_KEY,
					InvestsaudicustomerticketingConstants.FORM_GLOBAL_ERROR));
		...
		catch (final UnsupportedAttachmentException usAttEx)
		{
			...
			map.put(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource()
					.getMessage(InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_ATTACHMENT_BLOCK_LISTED, new Object[]
		...
		catch (final RuntimeException rEX)
		{
			...
			map.put(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource().getMessage(
					InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null, getI18nService().getCurrentLocale()));
			return new ResponseEntity<>(Lists.newArrayList(map), HttpStatus.BAD_REQUEST);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.UPDATE_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(InvestsaudicustomerticketingConstants.UPDATE_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_UPDATE));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(InvestsaudicustomerticketingConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(InvestsaudicustomerticketingConstants.MAX_UPLOAD_SIZE_MB,
		...
		try
		{
			...
			model.addAttribute(InvestsaudicustomerticketingConstants.SUPPORT_TICKET_DATA, ticketData);
		}
		catch (final Exception e)
		{
			LOG.error("Attempted to load ticket details that does not exist or is not visible", e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null);
	...
	@RequestMapping(value = "/childOrg", method = RequestMethod.POST)
	public String submitCreateChildOrgPage(final SagiaNewChildOrgForm form,
	...
	protected List<Breadcrumb> getBreadcrumbs(final String breadcrumbCode)
	{
		...
		breadcrumbs.add(new Breadcrumb("/my-account/support-tickets", getMessageSource().getMessage(
				InvestsaudicustomerticketingConstants.TEXT_SUPPORT_TICKETING_HISTORY, null, getI18nService().getCurrentLocale()), null));
		breadcrumbs.add(
