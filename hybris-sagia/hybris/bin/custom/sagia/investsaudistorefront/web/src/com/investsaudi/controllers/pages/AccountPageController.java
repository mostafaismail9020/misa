...
import com.investsaudi.forms.SagiaNewChildOrgForm;
import com.investsaudi.forms.SagiaNewUserForm;
import com.investsaudi.forms.SagiaUserForm;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	public static final String USER_OPERATION_LOCK = "inactive";
	public static final String USER_OPERATION_UNLOCK = "active";
	public static final String USER_OPERATION_CLOSE = "closed";
   ...
   private UserService userService;
	...
	@RequestMapping(value = "/child-unit", method = RequestMethod.POST)
	public String submitCreateChildUnitPage(final SagiaNewChildOrgForm form,
										   final BindingResult bindingResult, final Model model) throws CMSItemNotFoundException
	{
		SagiaB2BUnitData b2BUnitData = new SagiaB2BUnitData();
		b2BUnitData.setId(form.getUid());
		b2BUnitData.setName(form.getName());
		try {
			b2bRegistrationFacade.createChildUnit(b2BUnitData);
			model.addAttribute("createSuccess",true);
		} catch (ModelSavingException msException) {
			LOG.error(String.format("Not able to save B2BUnit with Id %s as this ID already exists", form.getUid()));
			model.addAttribute("newChildUnitForm", form	);
			model.addAttribute("errorMessageKey","uid.already.in.use");
		}
		return showCreateChildUnitPage(model);
	}

	@RequestMapping(value = "/child-unit", method = RequestMethod.GET)
	public String showCreateChildUnitPage(final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(ADD_NEW_CHILD_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ADD_NEW_CHILD_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
		accountBreadcrumbBuilder.getBreadcrumbs(ADD_NEW_CHILD_PAGE));
		model.addAttribute("existingChildUnits", b2bRegistrationFacade.getChildUnitsForCurrentUser());
		model.addAttribute("currentB2bUnt", b2bRegistrationFacade.getB2BUnitforCrurrentUser());
		if(!model.containsAttribute("newChildUnitForm")) {
			model.addAttribute("newChildUnitForm", new SagiaNewChildOrgForm());
		}
		return getViewForPage(model);
	}

	@RequestMapping(value = "/new-user", method = RequestMethod.POST)
	public String submitNewUserPage(final SagiaNewUserForm form,
										   final BindingResult bindingResult, final Model model) throws CMSItemNotFoundException
	{
		SagiaUserData userData = new SagiaUserData();
		userData.setEmail(form.getEmail());
		userData.setFirstName(form.getFirstName());
		userData.setLastName(form.getLastName());
		userData.setTitle(form.getTitle());
		userData.setParentUnit(form.getParentUnit());
		userData.setRole(form.getRole());
		userData.setMobileNumber(form.getMobileNumber());
		try {
			b2bRegistrationFacade.createUser(userData);
			model.addAttribute("createSuccess",true);
		} catch (PhoneNumberUsedException e) {
			LOG.error(String.format("Not able to save user with email %s as Mobile# %s already register with other user", userData.getEmail(),userData.getMobileNumber()));
			model.addAttribute("newUserForm", form);
			model.addAttribute("errorMessageKey","phone.already.in.use");
		} catch (CustomerAlreadyExistsException e) {
			LOG.error(String.format("Not able to save user with email %s as this email already used by other user", userData.getEmail(),userData.getMobileNumber()));
			model.addAttribute("newUserForm", form);
			model.addAttribute("errorMessageKey","email.already.in.use");
		}
		return showNewUserPage(model);
	}

	@RequestMapping(value = "/new-user", method = RequestMethod.GET)
	public String showNewUserPage(final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(ADD_NEW_USER_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ADD_NEW_USER_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(ADD_NEW_USER_PAGE));
		model.addAttribute("parentUnits",b2bRegistrationFacade.getChildUnitsForCurrentUser());
		model.addAttribute("userGroups",b2bRegistrationFacade.getUserGroupForNewUser());
		model.addAttribute("titles", userFacade.getTitles());
		if(!model.containsAttribute("newUserForm")) {
			model.addAttribute("newUserForm", new SagiaNewUserForm());
		}
		return getViewForPage(model);
	}
	@RequestMapping(value = "/view-child-units", method = RequestMethod.GET)
	public String showChildUnitsPage(final Model model) throws CMSItemNotFoundException {
		storeCmsPageInModel(model, getContentPageForLabelOrId(VIEW_CHILD_UNIT_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(VIEW_CHILD_UNIT_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(VIEW_CHILD_UNIT_PAGE));
		model.addAttribute("existingChildUnits", b2bRegistrationFacade.getChildUnitsForCurrentUser());
		return getViewForPage(model);
	}
...
//		}
	...
	@RequireHardLogIn
	public String showManageChildB2BCustomerPage(final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(MANAGE_CHILD_B2B_CUSTOMERS));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(MANAGE_CHILD_B2B_CUSTOMERS));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(MANAGE_CHILD_B2B_CUSTOMERS));

		model.addAttribute("childB2BCustomers",sagiaUserFacade.getAllChildB2BCustomers());

		if(!model.containsAttribute("manageUserForm")) {
			model.addAttribute("manageUserForm", new SagiaUserForm());
		}
		return getViewForPage(model);
	}
	...
	@RequireHardLogIn
	@ResponseBody
	public List<Map<String, String>> lockChildAccount(@RequestBody List<String> userIds, final RedirectAttributes redirectModel)
	{
		return performUserAction(userIds, USER_OPERATION_LOCK);
	}

	@RequestMapping(value = "/unlock-account", method = RequestMethod.POST)
	@RequireHardLogIn
	@ResponseBody
	public List<Map<String, String>> unlockChildAccount(@RequestBody final List<String> userIds, final RedirectAttributes redirectModel)
	{
		return performUserAction(userIds, USER_OPERATION_UNLOCK);
	}

	private List<Map<String, String>> performUserAction(final List<String> userIds, final String userAction) {
		List<Map<String, String>> response = Collections.EMPTY_LIST;

		if(userIds.size() == 0) {
			LOG.error(String.format("Invalid userIds - %s", userIds));
		} else {
			response = new ArrayList<>();
			for (String uid: userIds) {
				try {
					if(!isUserInTheSameState(uid, userAction)) {
						if (userAction.equals(USER_OPERATION_LOCK)) {
							b2bUserFacade.disableCustomer(uid);
						} else if (userAction.equals(USER_OPERATION_UNLOCK)) {
							sessionService.executeInLocalView(new SessionExecutionBody() {
								@Override
								public void executeWithoutResult() {
									searchRestrictionService.disableSearchRestrictions();
									b2bUserFacade.enableCustomer(uid);
									searchRestrictionService.enableSearchRestrictions();
								}
							});
						}
						response.add(createResponse(uid, "text.account.childUnits.operation.status.success", userAction, SUCCESS_RESPONSE_STATUS));
					} else {
						response.add(createResponse(uid, "text.account.childUnits.operation.not.permitted", userAction, FAILED_RESPONSE_STATUS));
					}
				} catch (Exception e) {
					LOG.error("Error", e);
					response.add(createResponse(uid, "text.account.childUnits.operation.status.failed", userAction, FAILED_RESPONSE_STATUS));
				}
			}
		}
		return response;
	...
	private Map<String, String> createResponse(String uid, String messageKey, String userAction, String status) {
		Map<String, String> map = new HashMap<>();
		CustomerData customerData = getB2bUnit(uid);
		if(StringUtils.isNotEmpty(uid)) {
			map.put("uid", uid);
		}
		if(StringUtils.isNotEmpty(messageKey)) {
			String userStatus = getMessageSource().getMessage(
					String.format("text.account.childUnits.%s.status", userAction), null,
					getI18nService().getCurrentLocale());

			String[] params = new String[]{uid, userStatus};
			map.put("message", getMessageSource().getMessage(messageKey, params,
					getI18nService().getCurrentLocale()));
		}
		...
		if(customerData != null) {
			map.put("userStatus", String.valueOf(customerData.isActive()));
		}
		return map;
	}

	/**
	 * This method is used to get the Customer data with admin context.
	 * @param uid
	 * @return
	 */
	private CustomerData getB2bUnit(final String uid)
	{
		return sessionService.executeInLocalView(new SessionExecutionBody()
		{
			@Override
			public CustomerData execute()
			{
				searchRestrictionService.disableSearchRestrictions();
				CustomerData customerData = b2bUserFacade.getCustomerForUid(uid);
				searchRestrictionService.enableSearchRestrictions();
				return customerData;
			}
		});
	...
	private boolean isUserInTheSameState(String uid, String userAction) {
		CustomerData customerData = getB2bUnit(uid);
		return ((userAction.equals(USER_OPERATION_LOCK) && !customerData.isActive())
				|| (userAction.equals(USER_OPERATION_UNLOCK) && customerData.isActive()));

		//TODO Need to add the code to handle deleted user
	}
}
