...
public class SagiaResourceComponentFreeTextValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider
{
                                                ...
                                                final IndexedProperty indexedProperty)
    {
        ...
        final String freeTextValue = getFreeTextValue(investSaudiResourceComponentModel, language);


        if (freeTextValue != null)
        {
            addFieldValues(fieldValues, indexedProperty, language, freeTextValue);
        }

        return fieldValues;
    }

    private String getFreeTextValue(InvestSaudiResourceComponentModel investSaudiResourceComponent, LanguageModel language) {
        
        Locale local = new Locale(language.getIsocode());
   
        String freeText = investSaudiResourceComponent.getResourceTitle(local).toLowerCase();
        freeText = freeText + " ";
        freeText = freeText + investSaudiResourceComponent.getResourceShortInformation(local).toLowerCase();
        return freeText;
    }
