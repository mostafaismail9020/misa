...
public class LicenseCancellationSetPopulator extends ODataPopulator<LicenseCancellationSetData> {

	private ContentHDRPopulator contentHDRPopulator;
    @Override
    public void populate(ODataModel model, LicenseCancellationSetData licenseCancellationSetData) throws ConversionException
    {
        super.populate(model,licenseCancellationSetData);
        licenseCancellationSetData.setLicenseCancelToContentNav(readLicenseCancellationSetAttachmentsFrom(model.get()));
        licenseCancellationSetData.setLicenseCancelToTextNav(readLicenseCancellationSetMessagesFrom(model.get()));

    }

	private List<ContentHDRData> readLicenseCancellationSetAttachmentsFrom(Map<String, Object> map) {
		return contentHDRPopulator.readAttachmentByOdataProperty(map, "LicenseCancelToContentNav");
	}

    private List<LicenseToTextNavData> readLicenseCancellationSetMessagesFrom(Map<String, Object> map) {

        Object licreplMessagesEntries = map.get("LicenseCancelToTextNav");
        if (licreplMessagesEntries != null) {
            ...
            return messagesODataEntries.stream()
                    ...
                    .map(this::toLicenceCancellationMessagesDTO)
                    .collect(Collectors.toList());
        }
        return Collections.emptyList();
    }

    private LicenseToTextNavData toLicenceCancellationMessagesDTO(Map<String, Object> map) {
