...
public class FileUploadFilter extends OncePerRequestFilter {
    ...
    protected MultipartFilter getMultipartFilter(final String servletPath) {
        for (Map.Entry<String, MultipartFilter> urlFilterMappingEntry : getUrlFilterMapping().entrySet()) {
            if (getPathMatcher().match(urlFilterMappingEntry.getKey(), servletPath)) {
                return urlFilterMappingEntry.getValue();
            }
        }
        return null;
    }
