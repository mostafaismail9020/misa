...
import com.sap.ibso.eservices.storefront.filters.cms.CMSSiteFilter;
...
public class CmsPageBeforeViewHandler implements BeforeViewHandler
{
	...
	protected void addCmsComponentActions(final Set<String> actionJsFiles, final List<AbstractCMSComponentModel> cmsComponents,
			final List<String> dependantAddOns)
	{
		for (final AbstractCMSComponentModel cmsComponent : cmsComponents)
		{
			...
			for (final AbstractCMSActionModel action : actions)
			{
				...
				{
					actionJsFiles.add(StringUtils.lowerCase(action.getItemtype()) + ".js");
				}
			}
		}
	}
