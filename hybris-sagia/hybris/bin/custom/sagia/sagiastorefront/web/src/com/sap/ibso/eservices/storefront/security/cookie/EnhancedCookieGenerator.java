 ...
 */
package com.sap.ibso.eservices.storefront.security.cookie;
...
public class EnhancedCookieGenerator extends CookieGenerator
{
	...
	protected void setEnhancedCookiePath(final Cookie cookie)
	{
		if (!canUseDefaultPath())
		{
			...
			{
				String contextPath = extractOnlyContectPath(request.getContextPath());
	...
	/**
	 * 
	 * this method is to extract only contectPath, on the production sometime contextPath comes like /sagiastorefront/sagia/en which cause logout when user try to switch the language. Hence this method ensure /sagiastorefront as context path
	 * @return contextPath
	 */
	private String extractOnlyContectPath(String contextPath) {
		String[] contextPathAry = contextPath.split("/");
		if (contextPathAry.length > 2)
		{
			return "/"+contextPathAry[1];
		}
		return contextPath;
	}
}
