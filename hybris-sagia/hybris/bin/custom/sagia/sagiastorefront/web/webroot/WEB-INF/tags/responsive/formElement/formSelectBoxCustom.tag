...
<%@ attribute name="selectedDataValue" required="false" type="java.lang.String" %>
...
<div class="formSelectBox">
	<template:errorSpanField path="${path}">
		<ycommerce:testId code="LoginPage_Item_${idKey}">
			<form:select id="${escapedIdKey}" path="${escapedPath}" cssClass="js-select2-oneColumn ${fn:escapeXml(selectCSSClass)}" tabindex="${fn:escapeXml(tabindex)}" disabled="${disabled}" data-value="${fn:escapeXml(selectedDataValue)}">
				...
				<c:choose>
					...
					<c:otherwise>
						<form:options itemValue="${not empty itemValue ? itemValue :'code'}" itemLabel="${not empty itemLabel ? itemLabel :'name'}" htmlEscape="true"/>
					</c:otherwise>
				</c:choose>
			</form:select>
