                        	...
                        	<c:url value="/mediaCenter/news" var="currentYearUrl"/>                                                  	
                           	<c:choose>
                                <c:when test="${param.isCurrentYear eq 'false'}">
                                    <a class="nav-link" id="pills-year-tab" href="${currentYearUrl}" role="tab" 
                                    		aria-controls="pills-year" aria-selected="true"><spring:theme code="portal.media.current.news" text="Current Year"/></a>
                                </c:when>
                                <c:otherwise>
                                    <a class="nav-link active" id="pills-year-tab" href="${currentYearUrl}" role="tab" 
                                    		aria-controls="pills-year" aria-selected="true"><spring:theme code="portal.media.current.news" text="Current Year"/></a>
                                </c:otherwise>
                        	</c:choose>
						</li>
                     	<li class="nav-item">
                       		<c:url value="/mediaCenter/news?isCurrentYear=${ycommerce:encodeUrl('false')}" var="pastNewsUrl"/>
                            <c:choose>
                                 <c:when test="${param.isCurrentYear eq 'false'}">
                                     <a class="nav-link active" id="pills-news-tab" href="${pastNewsUrl}" role="tab" 
                                     		aria-controls="pills-news" aria-selected="false"><spring:theme code="portal.media.past.news" text="Past News"/></a>
                                 </c:when>
                                 <c:otherwise>
                                     <a class="nav-link " id="pills-news-tab" href="${pastNewsUrl}" role="tab" 
                                     		aria-controls="pills-news" aria-selected="false"><spring:theme code="portal.media.past.news" text="Past News"/></a>
                                 </c:otherwise>
                             </c:choose>                         	
                     	</li>
                	...
                	<div class="col-lg-12 col-md-12 col-sm-12">
                    	<ul class="pagination pg-darkgrey pagination-bottom mt-4">
	                        ...
	                        <c:if test="${hasPreviousPage}">
	                           	<a class="page-link waves-effect waves-light" href="${pastNewsUrl}${urlWithPage}${searchPageData.pagination.currentPage-1}">
		                        ...
		                        <c:choose>
		                            <c:when test="${currentPage eq param.page + 1}">
		                                <li class="page-item active"><a class="page-link waves-effect waves-light"
		                                        href=${pastNewsUrl}${urlWithPage}${currentPage-1}>${currentPage}</a></li>
		                            </c:when>
		                            <c:otherwise>
                            ...
                            <c:if test="${hasNextPage}">
                                <a class="page-link waves-effect waves-light" href="${pastNewsUrl}${urlWithPage}${searchPageData.pagination.numberOfPages-1}">
