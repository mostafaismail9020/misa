//modified library. Don't update this file!
"use strict";
...
var _icls = function _icls(elem, add1, add2) {
    var _s = function _s(el) {
...
var JSCalendarEvent = function () {
    ...
    _createClass(JSCalendarEvent, [{
        ...
        value: function buildElements() {
            this.monthElem = _a('div', _icls(this.options.classNames.viewMonth.entry));
            this.weekElem = _a("div", _icls(this.options.classNames.viewWeek.entry));
            this.dayElem = _a("div", _icls(this.options.classNames.viewDay.entry));
        }
    }, {
        key: "updateElements",
        value: function updateElements() {
            ...
            //        let monthDetails = this.options.classNames.viewMonth.details;
            var weekDetails = this.options.classNames.viewWeek.details;
            var dayDetails = this.options.classNames.viewDay.details;
            // Month element
            if (this.at) {
            ...
            if (this.color) {
                monthElem.style.background = this.color;
            }
            if (this.options.eventTypes && this.type && this.options.eventTypes[this.type]) {
                var cls = this.options.eventTypes[this.type].cls;
                if (typeof cls == "string") {
                    monthElem.classList.add(cls);
                    weekElem.classList.add(cls);
            ...
            // Week element
            if (this.displayname) {
                weekElem.classList.add(_icls(weekDetails.containsText));
                weekElem.textContent = this.displayname;
            } else if (this.html) {
                weekElem.innerHTML = this.html;
                weekElem.classList.add(_icls(weekDetails.containsHtml));
            } else {
                weekElem.classList.add(_icls(weekDetails.isPlain));
                weekElem.textContent = this.options.nonameEventVocab;
            }

            // weekElem.style.background = this.color || this.options.eventBackground;

            !this.at && weekElem.classList.add(_icls(weekDetails.noTime));
            !this.duration && weekElem.classList.add(_icls(weekDetails.noDuration));
            ...
            if (this.at) {
                ...
                var timefloat = weekElem.querySelector("." + weekDetails.timeLabel) || _a('span', weekDetails.timeLabel, weekElem);
            ...
            if (this.at) {
                ...
                dayElem.classList.add(dayDetails.noTime);
            }
            ...
            var dayText = dayElem.querySelector("." + dayDetails.nameLabel) || _a('div', dayDetails.nameLabel, dayElem);
            dayText.textContent = this.displayname || this.options.nonameEventVocab;

            if (this.at) {
                var timeText = dayElem.querySelector("." + dayDetails.timeLabel) || _a('div', dayDetails.timeLabel, dayElem);
...
var JSCalendar = function () {
    ...
    _createClass(JSCalendar, [{
        ...
        value: function createStructure() {
            ...
            this.elem.classList.add(_icls(this.options.classNames.calendar));

            this.controlbar = _a('div', this.options.classNames.control.bar, this.elem);
            this.tablewrapper = _a('div', this.options.classNames.wrapper, this.elem);
            this.table = _a('table', this.options.classNames.table, this.tablewrapper);

            this.viewcontroller = _a('div', _icls(this.options.classNames.control.widget, this.options.classNames.control.views), this.controlbar);
            this.centercontroller = _a('div', _icls(this.options.classNames.control.widget, this.options.classNames.control.label), this.controlbar);
            this.actioncontroller = _a('div', _icls(this.options.classNames.control.widget, this.options.classNames.control.actions), this.controlbar);

            this.options.views.forEach(function (v) {
                var word = _this2.options.viewsVocab[v] || v;
                var button = _a('button', _icls(_this2.options.classNames.control.button, _icls(_this2.options.classNames.control.enumButton.views).length > 0 ? _this2.options.classNames.control.enumButton.views + " " + _this2.options.classNames.control.enumButton.views + "-" + v : ""), _this2.viewcontroller);
            ...
            this.options.buttons.forEach(function (b) {
                ...
                var parent = _this2.actioncontroller;
                if ((typeof action === "undefined" ? "undefined" : _typeof(action)) == 'object') {
                    parent = action.parent && action.parent == "center" ? _this2.centercontroller : _this2.actioncontroller;
                    if (!_this2.titlecontroller && action.type == "title") {
                        _this2.titlecontroller = _a('h2', _icls(_this2.options.classNames.control.title), parent);
                    }
                    action = action.action ? action.action : false;
                }
                if (action) {
                    var button = _a('button', _icls(_this2.options.classNames.control.button, _icls(_this2.options.classNames.control.enumButton.actions).length > 0 ? _this2.options.classNames.control.enumButton.actions + " " + _this2.options.classNames.control.enumButton.actions + "-" + b : ""), parent);
                    button.addEventListener('click', function () {
            ...
            if (!this.titlecontroller) {
                this.titlecontroller = _a('h2', _icls(this.options.classNames.control.title), this.centercontroller);
            }
        }
    }, {
        key: "goNow",
        ...
        value: function renderDay() {
            ...
            var classNames = this.options.classNames.viewDay;

            var dayrow = _a("tr", _icls(classNames.row), this.table);
            var daycol = _a("td", _icls(classNames.col), dayrow);

            this.validateCell();
            var events = this.state.matrix[this.state.year][this.state.month][this.state.day];
            var noTimeWrap = _a('div', _icls(classNames.entryEmpty), daycol);
            var timetable = _a('div', _icls(classNames.entries.table), daycol);
            var eventwrap = _a('div', _icls(classNames.entries.wrapper), timetable);
            ...
            for (var h = 0; h < 24; h++) {
                for (var gapi = 0; gapi < gapsPerHour; gapi++) {
                    var minutesBlock = _a('div', _icls(classNames.entries.item, classNames.entries.enumItem + '-' + ++gapindex), timetable);
                    var timespan = _a('span', _icls(classNames.entries.itemLabel), minutesBlock);

                    minutesBlock.classList.add(gapi == 0 ? _icls(classNames.entries.enumItemHour) : _icls(classNames.entries.enumItemMinute));
        ...
        value: function renderWeek() {
            ...
            var classNames = this.options.classNames.viewWeek;
            var dayrow = _a("tr", _icls(classNames.row), this.table);
            var daycol = _a("td", _icls(classNames.col), dayrow);
            ...
            var _loop2 = function _loop2(i) {
                ...
                var daysep = _a('div', _icls(classNames.cellHead), daycol);
                daysep.textContent = _this6.options.daysVocab[w] + ", " + _this6.options.monthsVocab[m] + " " + d;

                var daycontainer = _a('div', _icls(classNames.cell), daycol);
                ...
                var eventCol = _a("div", _icls(classNames.entryEmpty), daycontainer);
        ...
        value: function renderMonth() {
            ...
            var month = JSCalendar.getDaysInMonth(this.state.year, this.state.month);
            var classNames = this.options.classNames.viewMonth;
            ...
            var titleRow = _a('tr', _icls(classNames.rowHead), this.table);
            for (var day = 0; day < this.options.daysVocab.length; day++) {
                var td = _a('td', _icls(classNames.cellHead), titleRow);
                if (this.options.daysVocabShort) {
                    var label = _a('span', _icls(classNames.cellHeadCellLabel), td);
                    label.setAttribute("title", this.options.daysVocab[day]);
                    label.textContent = this.options.daysVocabShort[day];
                } else {
            ...
            for (var row = 0; row < 6; row++) {
                var tr = _a('tr', _icls(classNames.row, classNames.enumRow + '-' + row), this.table);

                var _loop3 = function _loop3(col) {
                    var date = new Date(_this7.state.year, _this7.state.month, ++startAt);

                    var td = _a('td', _icls(classNames.cell, classNames.enumRow + '-' + row, classNames.enumCol + "-" + col), tr);
                    var inMonth = true;
                    var daySpan = _a('span', _icls(classNames.number), td);
                    if (startAt <= 0 || startAt > totalDays) {
                        var _inMonth = false;
                        td.classList.add(_icls(classNames.cellInvalid));
                        daySpan.textContent = date.getDate();
                        if (row > 1 && col < 1) {
                            tr.classList.add(_icls(classNames.cellInvalid));
                        }
                    } else {
                        daySpan.textContent = date.getDate();
                    }

                    isCurrentMonth && currentDay == startAt && td.classList.add(_icls(classNames.cellCurrent));
                    tr.appendChild(td);
        ...
        value: function updateControls() {
            ...
            var button = this.controlbar.querySelector("." + _icls(this.options.classNames.control.enumButton.views) + "-" + this.state.view);
            button && button.classList.add('active');
        }
    }, {
        key: "appendLegend",
        value: function appendLegend() {
            if (this.options.eventTypes) {
                var legend = this.elem.getElementsByClassName(_icls(this.options.classNames.legend));
                if (legend.length == 0) {
                    legend = _a("ul", _icls(this.options.classNames.legend), this.elem);
                } else {
                    return;
                }
                for (var i in this.options.eventTypes) {
                    var entry = this.options.eventTypes[i];
                    if (entry.cls && entry.name) {
                        var lentry = _a("li", entry.cls, legend);
                        lentry.innerHTML = entry.name;
                    }
                }
            }
        }
        ...
        value: function applyCSS() {
            ...
            var cols = this.elem.querySelectorAll("." + _icls(this.options.classNames.viewMonth.cell));
        ...
        get: function get() {
            return {
                ...
                buttonsVocab: {
                    previous: '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="7px" height="12.002px" viewBox="-293.892 426.501 7 12.002" enable-background="new -293.892 426.501 7 12.002" xml:space="preserve"><path fill="#5CC83B" d="M-287.068,427.791c0.295-0.296,0.295-0.773,0-1.069c-0.295-0.295-0.774-0.295-1.069,0l-5.754,5.758l5.8,5.803c0.296,0.295,0.775,0.295,1.069-0.002c0.295-0.295,0.295-0.773,0-1.068l0,0l-4.731-4.732L-287.068,427.791L-287.068,427.791z"/></svg>',
                    today: "Today",
                    next: '<svg version="1.1" id="Ebene_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="7px" height="12.002px" viewBox="-293.892 426.501 7 12.002" enable-background="new -293.892 426.501 7 12.002" xml:space="preserve"><path fill="#5CC83B" d="M-293.624,437.213c-0.295,0.296-0.295,0.773,0,1.069c0.295,0.295,0.774,0.295,1.069,0l5.754-5.758l-5.8-5.803c-0.296-0.295-0.775-0.295-1.069,0.002c-0.295,0.295-0.295,0.773,0,1.068l0,0l4.731,4.732L-293.624,437.213L-293.624,437.213z"/></svg>'
                },
                ...
                classNames: {
                    ...
                    viewMonth: {
                        rowHead: "js-cal-weekday-row",
                        row: "js-cal-row",
                        cellHead: "js-cal-weekday",
                        cellHeadCellLabel: "cal-weekday-label",
                        cell: "js-cal-col",
                        number: "cell-number",
                        entry: "cell-event-mark",
                        cellInvalid: "notinmonth",
                        cellCurrent: "cell-today",
                        enumRow: "cal-row",
                        enumCol: "cal-col"
                    },
                    viewWeek: {
                        row: "cal-week-day-row",
                        col: "cal-week-day-col",
                        cellHead: "cal-week-day-sep",
                        cell: "col-week-day-container",
                        entry: "cal-week-day-event-col",
                        entryEmpty: "cal-week-day-no-event-col",
                        details: {
                            containsHtml: "html-injected",
                            containsText: "text-injected",
                            isPlain: "not-injected",
                            noDuration: "no-duration",
                            noTime: "no-starting-time",
                            timeLabel: "cal-week-day-time-float"
                        }
                    },
                    viewDay: {
                        row: "cal-week-day-row",
                        col: "cal-week-day-col",
                        entries: {
                            table: "single-day-time-table",
                            wrapper: "cal-total-day-event-wrap",
                            item: "hour-block",
                            enumItem: "gap",
                            enumItemHour: "hour-gap",
                            enumItemMinute: "minutes-gap",
                            itemLabel: "minutes-block-time"
                        },
                        entryEmpty: "single-day-no-time-wrap",
                        entry: "cal-total-day-event-view",
                        details: {
                            nameLabel: "cal-total-day-displayname",
                            timeLabel: "cal-total-day-time",
                            noDuration: "no-duration",
                            noTime: "no-time"
                        }
                    },
                    control: {
                        bar: "calendar-control-bar",
                        widget: "calendar-control-widget",
                        actions: "control-bar-actions",
                        views: "control-bar-actions control-bar-views",
                        label: "calendar-control-center",
                        button: "control-button",
                        enumButton: {
                            views: "change-view",
                            actions: "calendar-action"
                        },
