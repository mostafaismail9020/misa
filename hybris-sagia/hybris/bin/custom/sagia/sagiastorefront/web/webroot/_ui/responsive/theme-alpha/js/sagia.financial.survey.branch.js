SAGIA.financialSurvey.branch = SAGIA.financialSurvey.branch || {};
...
var setupBranchesTable = function() {
    ...
    SAGIA.financialSurvey.branch.dataTable = $('#branchesTableId').DataTable(config);
};
...
var editBranch = function () {
    ...
    var selectedBranch = financialSurvey.branches.find(function (branch) {
...
function updateBranchForm(selectedBranchId) {
    ...
    $('#volumeWeightId').val(selectedBranch.volumeWeight);
...
var saveBranch = function () {
    ...
    var volumeWeight = $('#volumeWeightId').val();
    ...
    if (branchSrId) { // edit branch
        ...
        var branch = financialSurvey.branches[branchIndex];
        if (branch.main) {
            branch.action = '02';
        ...
        branch.volumeWeight = volumeWeight;
    } else { // new branch
...
$("#volumeWeightId").keypress(function (e) {
    //if the letter is not digit then display error and don't type anything
    if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
        //display error message
        $("#errmsg").html("Digits Only").show().fadeOut("slow");
