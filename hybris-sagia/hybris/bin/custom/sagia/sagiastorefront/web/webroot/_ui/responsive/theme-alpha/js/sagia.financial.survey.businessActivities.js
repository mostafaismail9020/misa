...
$(function () {
    ...
    SAGIA.financialSurvey.businessActivities = {
        ...
        clearAll: function() {
            ...
            SAGIA.financialSurvey.businessActivities.selectedSection = null;
            SAGIA.financialSurvey.businessActivities.selectedDivisions = [];
            SAGIA.financialSurvey.businessActivities.selectedGroups = [];
            SAGIA.financialSurvey.businessActivities.selectedClasses = [];
            SAGIA.financialSurvey.businessActivities.selectedBranches = [];
            SAGIA.financialSurvey.businessActivities.selectedActivities = [];
            clearData();
        },
        ...
        ///        return;
            ...
            SAGIA.financialSurvey.businessActivities.licenseType = licenseTypeValue;
            $("#businessActivitiesSection #businessActivitiesModal #licenseTypeSpan").html(licenseTypeText ? licenseTypeText : licenseTypeValue);
            $.ajax(ACC.config.encodedContextPath + controllerUrl + "/complete/isic/" + licenseTypeValue, {
                type: "GET",
                ...
                success: function (data) {
                    ...
                    if(SAGIA.financialSurvey.apply.data && (SAGIA.financialSurvey.apply.data.licenseType == SAGIA.config.temporaryLicenseConstant))
                    {
                    	licenseAmendmentCurrentActivities = null;
                    	SAGIA.financialSurvey.apply.data.qeemahChannel = isicSections.activities[0].qeemahChannel;
                    }
    ...
    var setLicenseAmendmentCurrentActivities = function (currentActivities) {
        if (currentActivities && currentActivities.length) {
            ...
            if (currentActivities.length === currentActivitiesWithParentIds.length) { // Only valid activities: can show table and modal
                ...
                $('#sectionItems').hide();
                SAGIA.financialSurvey.apply.updateIsicTable();
            } else {
                currentActivities.forEach(function (activity) {
                    SAGIA.financialSurvey.businessActivities.selectedActivities.push({
                        activityId: activity.id,
                        description: activity.description,
                        splRequirementId: activity.splRequirementId,
                        qeemahChannel: activity.qeemahChannel
                    })
                    SAGIA.financialSurvey.businessActivities.existingActivities.push({
                        activityId: activity.id,
                        description: activity.description,
                        splRequirementId: activity.splRequirementId,
                        qeemahChannel: activity.qeemahChannel
            ...
            if(SAGIA.financialSurvey.apply.data)
             {
            	SAGIA.financialSurvey.apply.data.businessActivities = SAGIA.financialSurvey.businessActivities;
             }
        } else {
            if (!SAGIA.financialSurvey.businessActivities.selectedActivities.length > 1 && !$('body').hasClass('page-financial-survey')) {
    ...
    var _findIsicElement = function (type, dataIdWithValues) { //'selectedDivision', {"sectionId" : "5", "divisionId" : "4"}
        var isicElements = SAGIA.financialSurvey.businessActivities[type];
    ...
    var updateSections = function (uiSelectedSection, displaySections) {
        if (uiSelectedSection) {
            SAGIA.financialSurvey.businessActivities.selectedSection = ""+uiSelectedSection.data("sectionId");
            SAGIA.financialSurvey.businessActivities.selectedSectionName = uiSelectedSection.find("a").html();
            ...
            isicSections.divisions.forEach(function (division) {
                if (division.sectionId === SAGIA.financialSurvey.businessActivities.selectedSection &&
        ...
        if (displaySections) {
            ...
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedSection == null);
        }
    };
    ...
    var updateDivisions = function (displayDivisions) {
        ...
        if (uiSelectedDivisions.length) {
            ...
            SAGIA.financialSurvey.businessActivities.selectedDivisions = _updateIsicArray(uiSelectedDivisions, ["sectionId", "divisionId"]);
            var lis = "";
        ...
        if (displayDivisions) {
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedDivisions.length === 0);
    ...
    var updateGroups = function (displayGroups) {
        ...
        if (uiSelectedGroups.length) {
            ...
            SAGIA.financialSurvey.businessActivities.selectedGroups = _updateIsicArray(uiSelectedGroups, ["sectionId", "divisionId", "groupId"]);
            var lis = "";
        ...
        if (displayGroups) {
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedGroups.length === 0);
    ...
    var updateClasses = function (displayClasses) {
        ...
        if (uiSelectedClasses.length) {
            ...
            SAGIA.financialSurvey.businessActivities.selectedClasses = _updateIsicArray(uiSelectedClasses, ["sectionId", "divisionId", "groupId", "classId"]);
            var lis = "";
        ...
        if (displayClasses) {
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedClasses.length === 0);
    ...
    var updatedBranches = function (displayBranches) {
        ...
        if (uiSelectedBranches.length) {
            ...
            SAGIA.financialSurvey.businessActivities.selectedBranches = _updateIsicArray(uiSelectedBranches, ["sectionId", "divisionId", "groupId", "classId", "branchId"]);
            var lis = "";
        ...
        if (displayBranches) {
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedBranches.length === 0);
    ...
    var updateActivities = function (displayActivities) {
        ...
        if (uiSelectedActivities.length) {
            $("#businessActivitiesSection #businessActivitiesModal #activityList a.baBreadcrumb-link").removeClass("active");
            SAGIA.financialSurvey.businessActivities.selectedActivities = _updateIsicArray(uiSelectedActivities, ["sectionId", "divisionId", "groupId", "classId", "branchId", "activityId", "description","splrequirementId","qeemahChannel"]);
            var lis = "";
        ...
        if (displayActivities) {
            $("#businessActivitiesSection #businessActivitiesModal #nextButton").prop("disabled", SAGIA.financialSurvey.businessActivities.selectedActivities.length === 0);
            $("#businessActivitiesSection #businessActivitiesModal #activityItems.baModule").show();
        }
    };

    SAGIA.financialSurvey.apply.updateIsicTable = function () {
        ...
        $.each(SAGIA.financialSurvey.businessActivities.selectedActivities, function(i, e){
        	var matchingItems = $.grep(SAGIA.financialSurvey.businessActivities.newActivities, function (item) {
                return item.activityId === e.activityId;
             });
             if (matchingItems.length === 0){
             	SAGIA.financialSurvey.businessActivities.newActivities.push(e);
             }
        });

        SAGIA.financialSurvey.businessActivities.selectedActivities = SAGIA.financialSurvey.businessActivities.newActivities;


        SAGIA.financialSurvey.businessActivities.selectedActivities.forEach(function (activity) {
            if(isShowAttachments) {
            ...
            content += '' +
                ...
                '       <button class="btn btn_link deleteFromBusinessActivities">' +
    	...
    	SAGIA.financialSurvey.businessActivities.existingActivities.forEach(function (existingActivity) {
    ...
    $(document).on("change", "#businessActivitiesSection #businessType", function () {
        SAGIA.financialSurvey.businessActivities.businessType = $(this).val();
        SAGIA.financialSurvey.businessActivities.businessTypeName = $(this).find(":selected").text();
        $("#businessActivitiesSection #businessActivitiesModal #businessTypeSpan").parent().show();

        $("#businessActivitiesSection #businessActivitiesModal #businessTypeSpan").html(SAGIA.financialSurvey.businessActivities.businessTypeName);
        if (SAGIA.financialSurvey.businessActivities.selectedActivities.length) {
    ...
    $(document).on("click", "#businessActivitiesSection #businessActivitiesTable .deleteFromBusinessActivities", function () {
        ...
        trParent.remove();
        SAGIA.financialSurvey.businessActivities.selectedActivities.forEach(function (activity, index, arrayObject) {
