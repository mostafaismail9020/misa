(function () {
    ...
    $('.ratingModule-star.review').mouseenter(function () {
        ...
        if (index > 3) {
            ...
            $('#sendFeedbackButton').attr('disabled', false);
        } else {
            $('.formTextArea.formTextArea_slim').removeAttr('hidden');
            if ($.trim($('#extensionReason').val()).length < 1) {
                $('#sendFeedbackButton').attr('disabled', true);
            }
        }
    });

    $('#extensionReason').bind('input', function () {
        if ($.trim($('#extensionReason').val()).length < 1) {
            $('#sendFeedbackButton').attr('disabled', true);
        } else {
            $('#sendFeedbackButton').attr('disabled', false);
...
function sendFeedback() {
    var numberOfStars = 0,
        ...
        serviceId = SAGIA.chat.feedbackInProgress ? 'CHAT_FEEDBACK' : $('#serviceId').val(),
        extensionReason = $('#extensionReason').val();
    ...
    if (numberOfStars > 3 || (numberOfStars < 4 && extensionReason.length > 0)) {
        $.ajax({
            ...
            success: function() {
                SAGIA.chat.cleanupFeedbackModal();
            }
        });
    }
