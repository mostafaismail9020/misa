...
$(function () {
    ...
    function showAndSendFeedback() {
        ...
        var urlName;
        if(categoryUrl.startsWith("ignite"))
        {
          urlName="ignite";
        }
        else
        {
          urlName="government";
	        ...
	        type: "GET",
	        dataType: 'json',
	        url: ACC.config.encodedContextPath + "/services/"+ urlName + "/" + categoryUrl + "/" + serviceType + "/latest",
    ...
    // update the content in the details section, based on the selected item
        $('.serviceIgniteItem').click(function () {
            ajaxIgniteCallEvent($(this));
        });
    ...
    //update the content in the details section, based on the selected item
        function ajaxIgniteCallEvent(object) {
            var id = object.find(".historyList-id").text(),
                category = $('input#category').val(),
                service = $('input#service').val();
            $.ajax({
                type: "GET",
                dataType: 'json',
                url: ACC.config.encodedContextPath + "/services/ignite/" + category+ "/" + service + "/" + id,
                data: "id=" + id, // appears as $_GET['id'] @ your backend side
                success: function (data) {
                    var service = JSON.parse(data);
                    $("#currentID").text(service.srID);
                    if (service.attachments != null) {
                        ...
                        $('#igniteServicesAttachments').html('<ul class="downloadList downloadList_maxHeight">' + filesHtml + '</ul>');
                        ...
                        messagesListDivContent.empty();
                        //template messages
                        var messagesGovtServicesTemplate = document.getElementById("messagesSagiaIgniteServices-template");
                        var messagesGovtServicesTemplateHtml = messagesGovtServicesTemplate.innerHTML;

                        //messages from response
                        var messagesList = service.igniteServicesToTextNav;
                        var	messagesListHtml = '';
                        for(var key in messagesList) {
                        	messagesListHtml += messagesGovtServicesTemplateHtml
                    			.replace(
                        ...
                        $('#serviceUrl').html(
                            '<button class="btn btn_slim" ' +
                            'onclick="window.location.href=\'' +
                            ACC.config.encodedContextPath + '/services/ignite/' + category + "/" + serviceUrl + "/create" + '?serviceName=' + encodeURIComponent(serviceName) + '&amp;srID=' + service.srID +'\'\">' +
                            getI18nText("create.govtServices") +
                                '</button>'
                        );
                    }
